...
struct MapFeatureTests {
    ...
    func map_loadFavorites_populatesAndFetchesPrices() async {
        let store = TestStore(initialState: MapFeature.State()) {
            MapFeature()
        } withDependencies: {
            $0.apiClient = .mock()
    ...
    func map_loadFavorites_emptyDoesNotFetchPrices() async {
        let store = TestStore(initialState: MapFeature.State()) {
            MapFeature()
        } withDependencies: {
            $0.favoritesClient.all = { [] }
    ...
    func map_favoriteDisplays_ordersByPriceAndFlagsCheapest() async {
    ...
    func map_filterChange_refetchesFavoritePrices() async {
